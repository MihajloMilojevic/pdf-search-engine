...
class PhraseSearch(BaseSearch):
        ...
        for page in possible_pages:
            ...
            for word_result in words_result:
                if page not in word_result:
                    continue
            # find all indecies of the first word what is a start of the phrase
    ...
    def __str__(self):
        return f"PhraseSearch({' '.join([word.word for word in self.words])})"
